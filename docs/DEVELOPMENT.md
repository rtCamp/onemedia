# Development Guidelines - OneMedia
...
│
├── onemedia.php # Root plugin entrypoint.
...
- [Node.js](https://nodejs.org/) (and npm) + [NVM](https://nvm.sh/)
...
1. Clone the repository:
   ...
   ```bash
   git clone https://github.com/rtCamp/onemedia.git
   ```
...
- `npm run wp-env run cli -- --env-cwd=wp-content/plugins/onemedia {YOUR_CMD_HERE}`: Run WP-CLI commands in the local environment.
- `npm run wp-env:test run cli -- --env-cwd=wp-content/plugins/onemedia {YOUR_CMD_HERE}`: Run Composer/PHP tooling in the tests container.
...
Our specific ESLint configuration is defined in [eslint.config.mjs](../eslint.config.mjs) file.
...
OneMedia uses [Jest](https://jestjs.io/) for JavaScript/TypeScript unit testing, configured via [`jest.config.js`](../jest.config.js) and extending `@wordpress/scripts` defaults.
